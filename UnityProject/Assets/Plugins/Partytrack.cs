...
public class Partytrack {
	...
	[DllImport("__Internal")]
	private static extern void setCustomEventParameter_(string name, string svalue);
...
#endif
#if UNITY_ANDROID && !UNITY_EDITOR
        getTrack().CallStatic("start", getActivity(), app_id, app_key, getActivity().Call<AndroidJavaObject>("getIntent"));
#endif
    }
	...
	public static void setCustomEventParameter(string name, string svalue){
		#if UNITY_IPHONE && !UNITY_EDITOR
		setCustomEventParameter_(name, svalue);
		#endif
		#if UNITY_ANDROID && !UNITY_EDITOR
