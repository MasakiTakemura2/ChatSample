...
public class HelpListInfiniteLimitScroll : UIBehaviour, IInfiniteScrollSetup
{
	[SerializeField]
	...
	public IEnumerator Set ()
	{
		...
		Debug.Log ("count " + HelpListApi._httpCatchData.result.helps.Count);
		max = HelpListApi._httpCatchData.result.helps.Count;
	...
	{
		var infiniteScroll = GetComponent<HelpListInfiniteLimitScroll> ();
	...
	public void OnUpdateItem (int itemCount, GameObject obj)
	{
		if (itemCount < 0 || itemCount > max) 
		{
			...
			obj.SetActive (true);
			if (obj.GetComponentInChildren<HelpListItem> () != null)
			{
				var item = obj.GetComponentInChildren<HelpListItem> ();
				HelpListEntity.Helps itemObject = HelpListApi._httpCatchData.result.helps.ElementAt (itemCount);
                
				item.UpdateItem (itemCount, itemObject.quesstion, itemObject.answer);
			}
		}
