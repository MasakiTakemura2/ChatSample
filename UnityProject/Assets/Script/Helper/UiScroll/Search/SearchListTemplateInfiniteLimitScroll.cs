...
public class SearchListTemplateInfiniteLimitScroll : UIBehaviour, IInfiniteScrollSetup
{
	[SerializeField]
    ...
    private IEnumerator Set ()
    {
		...
		switch (SearchEventManager.Instance._currentSettingState) 
		{
			...
			case SearchEventManager.CurrentSettingState.BodyType:
				max = SearchModelManager.Instance.GetNameMaster (
					AppStartLoadBalanceManager._gender,
					SearchEventManager.CurrentSettingState.BodyType
				).Count;	
			break;
    ...
    {
		var infiniteScroll = GetComponent<SearchListTemplateInfiniteLimitScroll> ();
    ...
    public void OnUpdateItem (int itemCount, GameObject obj)
    {
        ...
        if (itemCount < 0 || itemCount >= max) 
		{
			...
			if (obj.GetComponentInChildren<SearchListDataSetItem> () != null)
            {
				switch(SearchEventManager.Instance._currentSettingState)
                {
					case SearchEventManager.CurrentSettingState.Sort:
					...
					case SearchEventManager.CurrentSettingState.BodyType:
						var bodyType = SearchModelManager.Instance.GetNameMaster
						(
							AppStartLoadBalanceManager._gender,
							SearchEventManager.CurrentSettingState.BodyType
						);
				...
				var item = obj.GetComponentInChildren<SearchListDataSetItem> ();
